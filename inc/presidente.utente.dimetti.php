...
<div class="row-fluid">
  <div class="span12">
    <div class="row-fluid">
        <h2><i class="icon-warning-sign muted"></i> Dimissione <?php if(isset($_GET['ordinario'])){ ?> Socio Ordinario <?php }else{ ?> Volontario <?php } ?></h2>
        <div class="alert alert-error">
          <div class="row-fluid">
            <span class="span12">
              <?php if($me->presiede()){?><p><strong>Presidente,</strong></p><?php } ?>
              <p>Stai dimettendo <strong><?php echo $v->nomeCompleto();?></strong>.</p>
    ...
    <div class="row-fluid">
      <?php if(isset($_GET['ordinario'])){ ?>
        <form class="form-horizontal" action="?p=presidente.utente.dimetti.ok&ordinario&id=<?php echo $v->id; ?>" method="POST">
      <?php }else{ ?>
